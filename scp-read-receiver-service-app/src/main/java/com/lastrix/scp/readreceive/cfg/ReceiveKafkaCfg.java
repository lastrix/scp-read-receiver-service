...
@Configuration
public class ReceiveKafkaCfg {
    private static final Logger log = LoggerFactory.getLogger(ReceiveKafkaCfg.class);
    ...
    private Map<String, Object> kafkaConsumerProperties() {
        ...
        cfg.put(ConsumerConfig.GROUP_ID_CONFIG, "read-receive");
...
//        cfg.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
    ...
    @Bean
    public ChangeReceiver<EnrolleeSelect> enrolleeSelectIdChangeReceiver(
            ConsumerFactory<String, String> factory,
            ObjectMapper mapper,
            @Value("${scp.kafka.topics.receive}") String topic) {
        return new KafkaChangeReceiver<>(factory.createConsumer(), x -> parseSafe(mapper, x), topic);
    }

    private EnrolleeSelect parseSafe(ObjectMapper mapper, String x) {
        try {
            return mapper.readValue(x, EnrolleeSelect.class);
